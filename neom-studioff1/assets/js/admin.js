  ...
  function playersSection() {
    ...
    const body = accs.length ? accs.map((a) => {
      ...
      return `<tr>
        ...
        <td><select class="mini-select" data-role="${escA(a.email)}" ${lockRole ? "disabled" : ""} title="${isOwner ? (mine ? "لا يمكن تغيير رتبتك" : "") : "المالك فقط يغيّر الرتب"}">${roleOpt(a.role || "member")}</select></td>
        ...
        <td class="act-cell">
          <button class="ico-btn sm danger" data-del-acc="${escA(a.email)}" ${(!isOwner || mine) ? "disabled" : ""} aria-label="حذف">${ic("trash")}</button>
        </td></tr>`;
