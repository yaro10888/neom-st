...
(function () {
  ...
  const avPath = (n) => `assets/img/avatars/avatar-${String(n).padStart(2, "0")}.svg`;
  ...
  function renderPicker() {
    ...
    if (!pick) return;
    pick.innerHTML = Array.from({ length: 10 }, (_, i) => i + 1)
      .map((n) => `<button data-av="${n}" class="${n === profile.avatar ? "sel" : ""}"><img src="${avPath(n)}" alt="avatar ${n}"/></button>`)
      .join("");
