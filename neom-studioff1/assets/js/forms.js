...
(function () {
  ...
  /* إذا أوقفت الإدارة استقبال الطلبات: عطّل الإرسال واعرض تنبيهًا (#9) */
  function gateApply(form) {
    ...
    if (!form.querySelector(".form-closed")) {
      ...
      n.textContent = (NEOM.i18n.lang === "en")
        ? "Applications are currently closed. Please check back later."
        : "باب التقديم مغلق حاليًا. تابعنا لاحقًا لإعادة فتحه.";
      form.prepend(n);
    }
  }
  ...
  function init() {
    ...
    if (apply) { initApply(apply); gateApply(apply); }
